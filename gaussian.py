...
class GaussianTransformer(object):
    ...
    def add_affinity(self, image, bbox_1, bbox_2):
        ...
        if (bbox_2[0][0] <= center_1[0] and center_1[0] <= bbox_2[2][0]) or(bbox_1[0][0] <= center_2[0] and center_2[0] <= bbox_1[2][0]):
            ...
            tl = np.mean([bbox_1[0], center_1], axis=0)
            bl = np.mean([bbox_1[1], center_1], axis=0)
            tr = np.mean([bbox_2[3], center_2], axis=0)
            br = np.mean([bbox_2[2], center_2], axis=0)

        else: #(bbox_1[0][1] <= center_2[1] and center_2[1] <= bbox_1[2][1]) or(bbox_2[0][1] <= center_1[1] and center_1[1] <= bbox_2[2][1]):
            '''tl = np.mean([bbox_1[0], bbox_1[1], center_1], axis=0)
            bl = np.mean([bbox_1[2], bbox_1[3], center_1], axis=0)
            tr = np.mean([bbox_2[0], bbox_2[1], center_2], axis=0)
            br = np.mean([bbox_2[2], bbox_2[3], center_2], axis=0)'''
            tl = np.mean([bbox_1[0], center_1], axis=0)
            bl = np.mean([bbox_1[3], center_1], axis=0)
            tr = np.mean([ bbox_2[1], center_2], axis=0)
            br = np.mean([bbox_2[2], center_2], axis=0)
        #else:
        #    return image
